...
function checkField(
  ...
  if (!extractedText.includes(normalizedValue)) {
    differences[fieldName] = `${fieldName} berubah / tidak ditemukan`;
  }
}
function checkAspek(
  extractedText: string,
  ...
  try {
    const parsed =
      typeof aspekData === "string" ? JSON.parse(aspekData) : aspekData;

    for (const item of parsed) {
      for (const val of Object.values(item)) {
        if (val && !extractedText.includes(normalizeText(String(val)))) {
          differences[fieldName] = `${fieldName} berubah`;
    ...
    console.error(`Parse ${fieldName} gagal`);
  }
}
...
console.log(
  ...
  : await prisma.result.findFirst({
    ...
    if (
      ...
      return NextResponse.json(
        {
          error: "Signature atau public key tidak ditemukan",
        },
        { status: 400 },
      );
    }
...
// //   stableStringify(verifyCanonical);
...
console.log(
  ...
  generatedHash === result.dataHash
    ...
    // const verifyCanonical = {
    //   nama: result.User?.fullName,

    //   pendidikan: result.User?.education,
...
// );
    // {
    //   // =============================
    //   // 🔹 KESIMPULAN PER KALIMAT
    //   // =============================

    //   const kesimpulanSentences =
    //     result.kesimpulan
    //       ?.split(/[.!?]/)
    //       .map((s) => normalizeText(s))
    //       .filter((s) => s.length > 15) || [];

    //   for (const sentence of kesimpulanSentences) {
    //     if (!extractedText.includes(sentence)) {
    //       differences.kesimpulan = "Kesimpulan berubah / tidak ditemukan";

    //       break;
    //     }
    //   }
    //   checkField(extractedText, value, key, differences);
    // }

    // checkAspek(extractedText, result.aspek1, "aspek1", differences);

    // checkAspek(extractedText, result.aspek2, "aspek2", differences);

    // checkAspek(extractedText, result.aspek3, "aspek3", differences);

    // checkAspek(extractedText, result.aspek4, "aspek4", differences);
...
function checkKeywords(
  ...
  const keywords = normalizeText(String(text))
    .split(" ")
    .filter((w) => w.length > 4);
  ...
  // minimal 70% keyword cocok
  if (ratio < 0.7) {
    differences[fieldName] =
      `${fieldName} berubah / tidak ditemukan`;
  }
}
    // =============================
    // STATUS KELAYAKAN
    ...
    // const hasLayak = compactText.includes("layak");

    // const hasTidakLayak = compactText.includes("tidaklayak");

    // const hasBelumLayak = compactText.includes("belumlayak");

    // if (result.layak && (!hasLayak || hasTidakLayak)) {
    //   differences.layak = "Status layak berubah";
    // }

    // if (result.tidakLayak && !hasTidakLayak) {
    //   differences.tidakLayak = "Status tidak layak berubah";
    // }

    // if (result.belumLayak && !hasBelumLayak) {
    //   differences.belumLayak = "Status belum layak berubah";
    // }
    // // =============================
    // VERIFY SIGNATURE ED25519
    // =============================
    const signatureUint8 = decodeBase64(result.Signature);
    const publicKeyUint8 = decodeBase64(result.ValidatedBy.PublicKey);
...
//     if (!isSignatureValid) {
//       message = "Signature digital tidak valid";
//     }
...
//   .createHash("sha512")
//   .update(
//     JSON.stringify(verifyCanonical)
//   )
//   .digest("hex");
//     //   const generatedHash = crypto
//     // .createHash("sha512")
