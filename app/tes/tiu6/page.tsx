...
import styles from "../cpmi/cpmi.module.css";
import { useRouter } from "next/navigation";
import TIU6Intro from "../../../components/TIU6/TIU6Intro";
import TIU6Instruction from "@/components/TIU6/TIU6Instruction";
...
const TIU6Page = () => {
  ...
  const [user, setUser] = useState<TIU6User | null>(null);
  ...
  const fetchUserAndTest = async () => {
    try {
      ...
      if (token) {
        ...
        // cek akses
        const accessRes = await fetch(`/api/tes/check-access?userId=${userData.user.id}&type=TIU6`);
      ...
      const testRes = await fetch("/api/tes/info?type=TIU6");
...
const loadQuestions = async (attemptId?: number) => {
  setLoading(true);
  try {
    const res = await fetch(`/api/tes?type=TIU6`, { credentials: "include" });
    ...
    qList = qList.filter((q) => !["TIU6-1", "TIU6-2", "TIU6-3", "TIU6-4", "TIU6-5", "TIU6-6", "TIU6-7", "TIU6-8", "TIU6-9", "TIU6-10"].includes(q.code));
   ...
   // setTimeLeft(testInfo?.duration ? testInfo.duration * 60 : 30 * 60);
    ...
    const startIndex = qList.findIndex((q) => q.code === "TIU6-11" || q.id === 469);
...
const handleSelectAnswer = async (
  ...
  return {
    ...
    choice: choice,
      ...
      const res = await fetch("/api/tes/submit-tiu6", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ userId: user.id, type: "TIU6", attemptId, answers: payload }),
        credentials: "include",
      });
      ...
      alert("🎉 Tes TIU6 selesai! Hasil bisa dilihat di Dashboard.");
...
setStep("questions");
  ...
  const loadExampleQuestions = async () => {
    try {
      const res = await fetch("/api/tes-tiu6-examples");
      const data = await res.json();
