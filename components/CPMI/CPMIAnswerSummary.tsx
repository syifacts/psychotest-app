...
interface AnswerSummaryProps {
  questions: Question[];
  answers: Record<string, string>; // Kalo di CPMI key-nya pake string (q.code)
  currentIndex: number;
  onSelect: (index: number) => void;
}

const CPMIAnswerSummary: React.FC<AnswerSummaryProps> = ({
  questions,
  answers,
  currentIndex,
  onSelect,
  ...
  return (
    <div className="bg-white/90 backdrop-blur-sm border border-blue-100 shadow-lg rounded-2xl p-6">
      ...
      {/* Grid responsif 5-6-8 kolom (PERSIS KAYAK MSDT) */}
      <div className="grid grid-cols-5 sm:grid-cols-6 md:grid-cols-8 gap-2 md:gap-3">
        {questions.map((q, idx) => {
          const isAnswered = !!answers[q.code]; // Beda di sini doang, cek pake q.code
          ...
          return (
            <button
              ...
              className={`
                ...
                ${
                  isCurrent
                    ? "ring-2 ring-offset-2 ring-blue-500 transform scale-110 z-10 relative" // +z-10 relative biar gak kepotong kotak sebelahnya
                    : "relative"
                }
                ${
                  isAnswered
...
export default CPMIAnswerSummary;
