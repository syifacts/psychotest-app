...
const paymentMethods = [
  ...
  { code: "MANDIRIVA", label: "Mandiri", logo: "/logos/mandiri.png" },
  { code: "BCAVA", label: "BCA", logo: "/logos/bca.png" },
  { code: "BNIVA", label: "BNI", logo: "/logos/bni.png" },
];
...
// 🔹 Inner: semua logic lama, termasuk useSearchParams
function CPMIPaymentInner({
  hasAccess,
  setHasAccess,
  startAttempt,
  testInfo,
  ...
  const [paymentStatus, setPaymentStatus] =
    useState<"PENDING" | "FREE" | "SUCCESS" | null>(null);
  const [tokenUser, setTokenUser] = useState<User | null>(null);
    ...
    const fetchUser = async () => {
      try {
        if (token) {
          ...
          const completed =
            Boolean(dataToken.isCompleted) || Boolean(dataToken.used);
          setTokenCompleted(completed);
          if (!completed) setHasAccess(true);
          setCheckingToken(false);
          return;
        ...
        if (resUser.ok && dataUser.user) {
          setUser({
            ...
            email: dataUser.user.email,
            role:
              dataUser.user.role === "PERUSAHAAN" ? "PERUSAHAAN" : "USER",
...
const handleSaveIdentity = async () => {
  ...
  // Modal identitas + metode pembayaran
  return (
    <>
      <Dialog open={open} onOpenChange={setOpen}>
        <DialogTrigger asChild>
...
// 🔹 Wrapper: dipakai di luar, membungkus inner dengan Suspense
const CPMIPaymentButton: React.FC<Props> = (props) => {
  return (
    <Suspense fallback={<p>Memuat tombol pembayaran...</p>}>
      <CPMIPaymentInner {...props} />
    </Suspense>
  );
};
