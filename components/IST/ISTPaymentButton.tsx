...
const ISTPaymentButton: React.FC<Props> = ({
  ...
  const [paymentStatus, setPaymentStatus] = useState<"PENDING" | "FREE" | "SUCCESS" | "FINISHED" | null>(null);
...
useEffect(() => {
  const checkPayment = async () => {
    ...
    try {
      ...
      const allFinished =
  data.payment.attempts?.length > 0 &&
  data.payment.attempts.every((a: any) => a.status === "FINISHED");
...
// ✅ 2️⃣ Kalau belum semua selesai, tapi payment sukses → boleh mulai tes
if (data.payment.status === "SUCCESS") {
  ...
  checkPayment();
}, [user, testInfo, setHasAccess, paymentCompleted]);
...
if (data.startTest || data.payment?.status === "FREE") {
 ...
 if (data.payment?.status === "SUCCESS") {
      setHasAccess(true);
      setPaymentStatus("SUCCESS");
        setPaymentCompleted(true); // ✅ tambahkan ini
...
if (paymentStatus === "SUCCESS" || paymentStatus === "FINISHED") {
  return (
      ...
      {attemptFinished ? (
        <>
          ...
          <p style={{ color: "gray", marginTop: "8px", fontSize: "14px" }}>
            Tes sebelumnya sudah selesai. Silakan bayar kembali untuk mengulang.
          </p>
        </>
      ) : (
        <button className={styles.btn} onClick={startAttempt}>
