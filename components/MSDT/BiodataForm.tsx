...
const BiodataForm: React.FC<Props> = ({ onSaved }) => {
  ...
  // Pisahin loading fetch awal dan loading pas submit
  const [isInitializing, setIsInitializing] = useState(true);
  const [isSubmitting, setIsSubmitting] = useState(false);

  useEffect(() => {
    const fetchBiodata = async () => {
      setIsInitializing(true); // Mulai loading pas baru buka form
      ...
      try {
        ...
        setIsInitializing(false); // Matiin loading kalau udah kelar fetch
      }
    };
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    // Hapus tujuan dari validasi karena di form-nya lu comment (tujuan disembunyiin)
    ...
    try {
      ...
      // Menghapus pop-up alert usia biar lebih clean transisinya
      if (onSaved) onSaved();
    } catch (err: any) {
      alert("❌ " + err.message);
    } finally {
  ...
  // 1. TAMPILAN LOADING (Mencegah user iseng ngisi)
  if (isInitializing) {
    return (
  ...
  // 2. TAMPILAN FORM UTAMA (Senada dengan MSDTInstruction)
  return (
      ...
      <div className="max-w-xl w-full bg-white/90 backdrop-blur-sm shadow-2xl rounded-3xl p-8 md:p-10 border border-blue-100 relative overflow-hidden">
        ...
        <div className="flex flex-col items-center mb-8 relative z-10">
          ...
          <p className="text-sm text-gray-500 mt-1">
            Pastikan data Anda sudah benar sebelum memulai tes
          </p>
        </div>
        ...
        <form onSubmit={handleSubmit} className="space-y-5 relative z-10">
          ...
          <div className="pt-4">
            <button
              ...
              className={`w-full py-3.5 rounded-xl text-white font-bold text-lg shadow-lg transition-all duration-300 transform hover:-translate-y-1 ${
                isSubmitting
                  ? "bg-blue-400 cursor-not-allowed"
                  : "bg-gradient-to-r from-blue-600 to-indigo-600 hover:shadow-blue-500/30"
              }`}
            >
              {isSubmitting ? "Menyimpan Data..." : "Lanjut ke Instruksi Tes"}
            </button>
          </div>
        </form>
      </div>
