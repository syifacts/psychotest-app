...
export default function ReportCPMIDocument({
  attempt,
  result,
  kesimpulan,
  kesimpulanSikap,
  ...
  // useEffect(() => {
  //   if (barcode) {
  //     const url = `https://3fcf-2001-448a-2020-5a6-172-2549-cda5-6f91.ngrok-free.app/validate/${barcode}`;
...
//     ) {
//       const url =
//         `https://3fcf-2001-448a-2020-5a6-172-2549-cda5-6f91.ngrok-free.app/validate/${barcode}`;
...
// }, [barcode, qrCodeBase64]);
  ...
  const kesimpulanSikapPDF = (kesimpulanSikap || "").replace(
    /{name}/g,
    attempt?.User?.fullName || "-",
  );

  const kesimpulanKepribadianPDF = (kesimpulanKepribadian || "").replace(
    /{name}/g,
    attempt?.User?.fullName || "-",
  );

  const kesimpulanBelajarPDF = (kesimpulanBelajar || "").replace(
    /{name}/g,
    attempt?.User?.fullName || "-",
  );
  ...
  const rows = [
    {
      ...
      uraian: "Kemampuan untuk berpikir secara logis dan sistematis.",
    },
    {
      no: "B",
      aspek: "Daya Analisis",
      uraian:
        "Kemampuan untuk melihat permasalahan dan memahami hubungan sebab-akibat permasalahan.",
    },
    {
      no: "C",
      aspek: "Kemampuan \nNumerikal",
    ...
    },
    {
      ...
      uraian:
        "Kemampuan untuk memahami konsep dan pola dalam bentuk kata-kata dan mengekspresikan gagasan secara verbal.",
    },
  ];
  const rows2 = [
    {
      no: "A",
      aspek: "Orientasi Hasil",
      uraian:
        "Kemampuan untuk mempertahankan komitmen menyelesaikan tugas secara bertanggungjawab dan memperhatikan keterhubungan antara perencanaan dan hasil kerja.",
    },
    {
      no: "B",
      aspek: "Fleksibilitas",
    ...
    {
      no: "C",
      aspek: "Sistematika Kerja",
      uraian:
        "Kemampuan untuk merencanakan hingga mengorganisasikan cara kerja dalam proses penyelesaian pekerjaan.",
    },
  ];
  const rows3 = [
    {
      no: "A",
      aspek: "Motivasi",
      uraian: "Kemampuan untuk menunjukkan prestasi dan mencapai target.",
    },
    {
      no: "B",
      aspek: "Kerjasama",
      uraian:
        "Kemampuan untuk menjalin, membina dan mengoptimalkan hubungan kerja yang efektif demi tercapainya tujuan bersama.",
    },
    {
      no: "C",
    ...
    },
    {
      ...
      uraian: "Kemampuan untuk memahami dan mengontrol emosi.",
    },
  ];
  const rows4 = [
    {
      ...
      uraian: "Kemampuan dalam menyesuaikan diri dengan situasi yang baru.",
    },
    {
      no: "B",
      aspek: "Pengembangan Diri",
      uraian:
        "Kemampuan meningkatkan pengetahuan dan penyempurnaan keterampilan diri.",
    },
  ];
    ...
    <Document>
      ...
      <Page size="A4" style={styles.page}>
        ...
        <View style={{ marginTop: 30, marginBottom: 30 }}>
          ...
          <Text style={{ fontSize: 17, color: "gray" }}>{nomorDokumen}</Text>
        </View>
      ...
      <Page size="A4" style={styles.page}>
        ...
        <View style={{ marginBottom: 20 }}>
          ...
          {/* Nama Psikolog */}
          <View
            style={{ flexDirection: "row", paddingTop: 4, paddingBottom: 4 }}
          >
            <Text style={{ width: 180, fontFamily: "Times-Bold" }}>
              Nama Psikolog
            </Text>
            <Text style={{ flex: 1 }}>
              {" "}
              : {result?.ValidatedBy?.fullName || "-"}
            </Text>
          </View>
          <Text
            style={{
          ...
          >
            ...
            <Text style={{ flex: 1 }}>
              {" "}
              : {result?.ValidatedBy?.lembagalayanan || "-"}
            </Text>
          </View>
          <Text
            style={{
          ...
          >
            ...
            <Text style={{ flex: 1 }}>
              {" "}
              :{" "}
              {new Date(result?.validatedAt || Date.now()).toLocaleDateString(
                "id-ID",
        ...
        <View style={{ marginTop: 10, borderWidth: 1, borderColor: "#000" }}>
          ...
          <View style={[styles.tableRow, { backgroundColor: "#d8b6d8" }]}>
            ...
            <Text
              style={[styles.cell, { width: 50, borderBottomWidth: 1 }]}
            ></Text>
            <Text
              style={[
                styles.cell,
                { width: 170, borderLeftWidth: 1, borderBottomWidth: 1 },
              ]}
            ></Text>
            <Text
              style={[
                styles.cell,
                { width: 400, borderLeftWidth: 1, borderBottomWidth: 1 },
              ]}
            ></Text>
          ...
          {rows.map((row) => {
            ...
            const kategoriObj = aspekScores.find(
              (a) => String(a.aspek) === String(row.no),
            );
            const kategoriNormalized = (kategoriObj?.kategori || "")
              .trim()
              .toUpperCase();
          ...
          >
            <View
              style={{ flexDirection: "row", marginLeft: 14, marginTop: 3 }}
            >
      ...
      <Page size="A4" style={styles.page}>
        ...
        <View style={{ marginTop: 10, borderWidth: 1, borderColor: "#000" }}>
          ...
          {/* Rows */}
          {rows2.map((row) => {
            const kategoriObj = aspekScores2.find(
              (a) => String(a.aspek) === String(row.no),
            );
            const kategoriNormalized = (kategoriObj?.kategori || "")
              .trim()
        ...
        <View style={{ marginTop: 10, borderWidth: 1, borderColor: "#000" }}>
          ...
          {/* Rows isi */}
          {rows3.map((row) => {
            const kategoriObj = aspekScores3.find(
              (a) => String(a.aspek) === String(row.no),
            );
            const kategoriNormalized = (kategoriObj?.kategori || "")
              .trim()
            ...
            return (
              ...
              >
                <Text
                  style={[
                    styles.cell,
                    { width: 50, textAlign: "center", fontSize: 12 },
                  ]}
                >
                  {row.no}
                </Text>
                <Text
                  style={[
                    styles.cell,
                    { width: 170, fontSize: 12, borderLeftWidth: 1 },
                  ]}
                >
                  {row.aspek}
                </Text>
      ...
      <Page size="A4" style={styles.page}>
        ...
        <View style={{ marginTop: 10, borderWidth: 1, borderColor: "#000" }}>
          ...
          {/* Rows */}
          {rows4.map((row) => {
            const kategoriObj = aspekScores4.find(
              (a) => String(a.aspek) === String(row.no),
            );
            const kategoriNormalized = (kategoriObj?.kategori || "")
              .trim()
  ...
  {new Date(result?.validatedAt || Date.now()).toLocaleDateString(
          ...
          >
            ...
            {/* Nilai */}
            <Text
              style={{ fontSize: 12, width: 150, fontFamily: "Times-Bold" }}
            >
          ...
          <Text style={{ fontSize: 10, color: "gray" }}>Licence Number</Text>
        </View>
        ...
        {new Date(expiresAt).toLocaleDateString("id-ID")}
            ...
            {validationNotes && (
              <View style={{ width: 120, marginTop: 1 }}>
                <Text style={[styles.validation, { textAlign: "justify" }]}>
                  {validationNotes}
                </Text>
              </View>
            )}
...
// export default function ReportCPMIDocument({ attempt, result, kesimpulan, kesimpulanSikap,kesimpulanBelajar,kesimpulanKepribadian, ttd, barcode, expiresAt, validationNotes, kesimpulanumum, saranpengembangan, dataHash, signature }: Props) {
//   const [qrCodeBase64, setQrCodeBase64] = useState<string>("");

//   useEffect(() => {
//     if (barcode) {
//   const url = `https://psiko.deltaindonesialab.com/validate/${barcode}`;
//       QRCode.toDataURL(url)
//         .then(setQrCodeBase64)
//         .catch((err) => console.error("QR generation error:", err));
//     }
